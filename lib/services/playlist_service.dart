...
class PlaylistService {
  ...
  Future<List<Playlist>> list({User? user}) async {
    final js = await api
        .get(user == null ? "playlists" : "users/${user.username}/playlists");
    return (js["data"] as List<dynamic>)
  ...
  Future<Playlist> updateImage(Playlist playlist, PlatformFile image) async {
    final js = await api
        .multipartRequest("POST", "playlists/${playlist.id}/photo", files: {
      "photo": image,
    });
